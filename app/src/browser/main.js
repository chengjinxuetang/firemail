...
const declareOptions = argv => {
  ...
  options.usage(
    `Firemail\n\nUsage: firemail [options] [recipient] [attachment]\n\nRun Firemail: The open source extensible email client\n\n\`firemail mailto:johndoe@example.com\` to compose an e-mail to johndoe@example.com.\n\`firemail ./attachment.txt\` to compose an e-mail with a text file attached.\n\`firemail --dev\` to start the client in dev mode.\n\`firemail --test\` to run unit tests.`
  );
  options
    .alias('d', 'dev')
    .boolean('d')
  ...
  options
    ...
    .string('c')
    .describe('c', 'Override the path to the Filemail configuration directory');
  options
  ...
  options
    .alias('b', 'background')
    .boolean('b')
    .describe('b', 'Start Filemail in the background');
  return options;
};
