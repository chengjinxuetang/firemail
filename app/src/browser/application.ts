...
export default class Application extends EventEmitter {
  ...
  async start(options) {
    ...
    try {
      ...
      if (err.toString().includes('ENOENT')) {
        message = localized(
          `Firemail could not find the mailsync process. If you're building Firemail from source, make sure mailsync.tar.gz has been downloaded and unpacked in your working copy.`
        );
      } else if (err.toString().includes('spawn')) {
        message = localized(`Firemail could not spawn the mailsync process. %@`, err.toString());
      } else {
        message = localized(
          `We encountered a problem with your local email database. %@\n\nCheck that no other copies of Firemail are running and click Rebuild to reset your local cache.`,
          err.toString()
        );
    ...
    if (process.platform === 'linux') {
      ...
      helper.registerForURLScheme('firemail');
    } else {
      app.setAsDefaultProtocolClient('firemail');
    }
  }
  ...
  runSpecs(specWindowOptionsArg) {
    ...
    const configDirPath = path.join(app.getPath('home'), '.firemail-spec');
